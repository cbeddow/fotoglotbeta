...
function classifyImage(image) {
  return new Promise((resolve, reject) => {
    ...
    tf.loadGraphModel('https://tfhub.dev/google/tfjs-model/imagenet/inception_v3/classification/5').then(model => {
      ...
      model.predict(resized.expandDims()).data().then(prediction => {
        // Get the top predicted label
        const topLabelIndex = Array.from(prediction).indexOf(Math.max(...prediction));
        const labels = fetch('https://storage.googleapis.com/tfjs-models/tfjs/mobilenet_v1_1.0_224_frozen.json')
          .then(res => res.json())
          .then(data => {
